...
class WSCP
{
  public:
    ...
    int select_set_Sparrow(int var_index);
    int select_set_TabuSparrow();
    int select_set_Greedy();
    int select_set_ExploreSparrow();
    int select_novelty_explore_set();
    int select_set2();
    int select_set_from_zero_stack();
    int select_uncover_set();
    //void set_param(int new_tabu_length, double new_novelty_p, double new_explore_p, int new_t);
...
/*
void WSCP::set_param(int new_tabu_length, double new_novelty_p, double new_explore_p, int new_t)
{
    tabu_length = new_tabu_length;
    novelty_p = new_novelty_p;
    explore_p = new_explore_p;
    t = new_t * 10;
}
*/
...
int WSCP::compareV3(int s1, int c1, int s2, int c2, int p1, int p2, int weight1, int weight2)
{
    ...
    {   
        int balance = gap1 * comp_comb_score + gap2 * comp_score;
        int heuristic_result;
        if (balance > 0)
            heuristic_result = 1;
        else if (balance = 0)
            heuristic_result = 0;
        ...
        /*
        if ((random() % 100)/(double)101 > explore_rate)
            return comp_score;
        else
...
int WSCP::select_novelty_explore_set()
{
    int sr = INT_MIN, ct = 1, i;
    int best_pscore = INT_MIN;
    int best_set = -1;
    ...
    if (t < set_count)
    {
        for (int j = 0; j < t; ++j)
        {
            ...
            {
                if (pscore[sec_best_set] < pscore[i])
                    sec_best_set = i;
                else if (pscore[sec_best_set] == pscore[i] && time_stamp[sec_best_set] > time_stamp[i])
                    sec_best_set = i;
            }
        }
    }
    ...
    if ((random() % 100)/(double)101 < explore_p)
        best_set = sec_best_set;

    return best_set;
}

int WSCP::select_set_Greedy()
{
    int selected_uncover_var;
    int sr = INT_MIN, ct = 1, best_set = 0;
    int ps = INT_MIN;
    for (int j = 0; j < uncover_stack_fill_pointer; ++j)
    {
        selected_uncover_var = uncover_stack[j];
        for (int i = 0; i < var_set_num[selected_uncover_var]; ++i)
        {
            int cur_set = var_set[selected_uncover_var][i];
...
int WSCP::select_set_Novelty()
{
    ...
    for (int i = 0; i < var_set_num[selected_uncover_var]; ++i)
    {
        int cur_set = var_set[selected_uncover_var][i];
        /*
        if (step - time_stamp[sec_best_set] < tabu_length)
            continue;
        */
        if (cur_set == best_set)
            continue;
...
int WSCP::select_set_ExploreSparrow()
{
    /*
    ...
    for (int i = 0; i < var_set_num[min_weight_var]; ++i)
    {
        int cur_set = var_set[min_weight_var][i];
        if (score[cur_set] < min_score)
        {
            min_score = score[cur_set];
            min_score_set = cur_set;
        }
    }

    if (cur_solu[set_solu[min_score_set]] == 0 || fix[set_solu[min_score_set]])
        return select_set(0);

    return min_score_set;
    */

    double *comb_score_list = new double[set_count];
    int *age_list = new int[set_count];
    double *prob_dist = new double[set_count];
    double *cdf_dist = new double[set_count];
    double *covered_set_list = new double[set_count];
    double denominator = 0;
    int covered_set_num = 0;

    for (int i = 0; i < set_count; ++i)
    {
        int cur_set = set_solu[i];
        if (!cur_solu[cur_set])
            continue;
        if (fix[cur_set])
            continue;
        
        covered_set_list[covered_set_num] = cur_set;
        comb_score_list[covered_set_num] = exp(weight1*score[cur_set] + weight2*pscore[cur_set]);
        denominator += comb_score_list[covered_set_num];
        covered_set_num++;
    }

    for (int i = 0; i < covered_set_num; ++i)
    {
        prob_dist[i] = comb_score_list[i]/denominator;
        if (i == 0)
            cdf_dist[i] = prob_dist[i];
        else
            cdf_dist[i] = cdf_dist[i-1] + prob_dist[i];
    }

    double rand_point = (random() % 1000)/(double)1001;
    int rand_index;

    for (int i = 0; i < covered_set_num; ++i)
    {
        if (rand_point <= cdf_dist[i])
        {
            rand_index = i;
            break;
        }
    ...
    int choosen_set = covered_set_list[rand_index];

    
    cout << "choosen_set: " << choosen_set << endl;
    cout << "rand_point: " << rand_point << endl;
    cout << "rand_index: " << rand_index << endl;

    cout << "cdf_dist_first: " << cdf_dist[0] << endl;
    cout << "cdf_dist_last: " << cdf_dist[covered_set_num-1] << endl;
    

    delete covered_set_list;
    delete comb_score_list;
    delete prob_dist;
    delete cdf_dist;
    delete age_list;
...
int WSCP::select_set_TabuSparrow()
{
    int selected_uncover_var = uncover_stack[random() % uncover_stack_fill_pointer];
    int sr = INT_MIN, ct = 1, best_set = 0;
    int ps = INT_MIN;
    for (int i = 0; i < var_set_num[selected_uncover_var]; ++i)
    {
        int cur_set = var_set[selected_uncover_var][i];
        if (step - time_stamp[cur_set] < tabu_length)
            return select_set_Sparrow(selected_uncover_var);
        if (sr == INT_MIN || compareV2(sr, ct, score[cur_set], cost[cur_set], ps, pscore[cur_set], weight1, weight2) < 0)
        {
            sr = score[cur_set];
            ps = pscore[cur_set];
...
/*
int WSCP::select_set_Sparrow(int var_index)
{
    int selected_uncover_var;
    if (var_index == -1)
        selected_uncover_var = uncover_stack[random() % uncover_stack_fill_pointer];
    else
        selected_uncover_var = var_index;

    double *comb_score_list = new double[var_set_num[selected_uncover_var]];
    int *age_list = new int[var_set_num[selected_uncover_var]];
    double *prob_dist = new double[var_set_num[selected_uncover_var]];
    double *cdf_dist = new double[var_set_num[selected_uncover_var]];
    double denominator = 0;
    double c1 = 2;
    ...
    for (int i = 0; i < var_set_num[selected_uncover_var]; ++i)
    {
        ...
        comb_score_list[i] = exp(weight1*score[cur_set] + weight2*pscore[cur_set]);
        //comb_score_list[i] = weight1*score[cur_set] + weight2*pscore[cur_set];
        age_list[i] = step - time_stamp[cur_set];

        denominator += comb_score_list[i];
    }
    ...
    int choosen_set = var_set[selected_uncover_var][rand_index];
...
int WSCP::select_set_Sparrow(int var_index)
{
    ...
    double rand_point = (random() % 1000)/(double)1001;
    double rand_point_value = rand_point * denominator;
    denominator = 0;
    int rand_index;
    ...
    {
        denominator += comb_score_list[i];
        if (denominator >= rand_point_value)
        {
            rand_index = i;
            break;
        }
...
void WSCP::local_search()
{
    for (tries = 0; tries < max_tries; ++tries)
    {
        for (step = 0; step < max_step; ++step)
        {
            ...
            if (uncover_stack_fill_pointer > 0)
            {
                //int best_set = select_uncover_set();
                //int best_set = select_set_Greedy();
                int best_set = select_set_Novelty();
                //int best_set = select_set_Sparrow(-1);
                //int best_set = select_set_TabuSparrow();

                if (cur_cost + cost[best_set] >= best_cost)
                {
                    int flip_set;
                    flip_set = select_set(0);
                    //flip_set = select_set_ExploreSparrow();
                    //flip_set = select_novelty_explore_set();
                    if (flip_set == -1)
                        return;
                    if (flip_set < 0)
                    {
            ...
            //int flip_set = select_set_ExploreSparrow();
            //int flip_set = select_novelty_explore_set();
